...
// A later rule that also matches this remote can never fire — the earlier
// rule above already claimed it (first match wins). Surface that footgun.
function warnShadowed(r) {
  if (!r.shadowed || r.shadowed.length === 0) return;
  console.log(
    yellow(`  ⚠ ${r.shadowed.length} later rule${r.shadowed.length > 1 ? 's' : ''} also match this remote but are shadowed (first match wins):`)
  );
  for (const { rule, index } of r.shadowed) {
    console.log(dim(`      rule #${index + 1}: ${rule.remote}`));
  }
  console.log(dim('      reorder your rules (most specific first) if that is not what you want'));
}
...
export function check({ hook = false } = {}) {
  ...
  switch (r.status) {
    ...
    case 'mismatch':
      ...
      console.error(`  remote  : ${r.remote ? `${r.remote.host}/${r.remote.path}` : '(no origin remote)'}`);
...
// Predict whether the installed hook will actually be able to resolve
// gitsignet at commit time. The hook tries `command -v gitsignet` then
// `npx --no-install gitsignet` — crucially, `--no-install` does NOT consult
// the npx download cache, so a user who only ever ran `npx gitsignet ...`
// has no resolvable binary and the hook silently falls open (issue #2).
// We run the same probes here so `install` can warn loudly.
function hookCanResolveBinary() {
  const probe = (cmd) => {
    try {
  ...
  return probe('command -v gitsignet') || probe('npx --no-install gitsignet --version');
}
...
export function install() {
  ...
  if (!hookCanResolveBinary()) {
    console.error('');
    console.error(yellow('  ⚠ gitsignet is not installed where the hook can find it.'));
    console.error(yellow("    The guard will FALL OPEN — commits pass unchecked — until you install it."));
    console.error(dim("    Running via `npx gitsignet` does NOT count: the hook uses `npx --no-install`,"));
    console.error(dim('    which cannot see the npx download cache. Install a resolvable binary:'));
    console.error(dim('      npm i -g gitsignet        # global, works everywhere'));
    console.error(dim('      npm i -D gitsignet        # per-project dev dependency'));
  }
  return 0;
}
