...
import { isGitRepo, repoRoot, hooksDir } from './git.js';
...
export function doctor() {
  ...
  switch (r.status) {
    ...
    case 'mismatch':
      ...
      console.log(dim('  fix: git config user.name / user.email, or `git config` --global'));
      return 1;
    case 'no-config':
...
export function check({ hook = false } = {}) {
  ...
  switch (r.status) {
    ...
    case 'mismatch':
      ...
      console.error(dim('  fix    : git config user.name "…" && git config user.email "…"'));
      return 1;
    case 'no-identity':
