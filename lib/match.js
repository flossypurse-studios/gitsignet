...
// Find the first rule that explicitly targets the "no origin remote" case.
// A rule opts in by setting its `remote` to the literal string "(none)".
// This lets a repo with no remote still be guarded (see issue #1) — the
// "(none)" token never matches a real remote because parseRemote candidates
// are always "host/..." strings.
export function matchNoRemote(rules) {
  if (!Array.isArray(rules)) return null;
  for (const rule of rules) {
    if (!rule || typeof rule.remote !== 'string') continue;
    if (rule.remote.trim().toLowerCase() === '(none)') return rule;
  }
  return null;
}
