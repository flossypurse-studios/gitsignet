...
export function evaluate({ remote = 'origin' } = {}) {
  ...
  if (!identity.name || !identity.email) {
    return { status: 'no-identity', identity, remote: parsed, url, configPath, config };
  }

  if (!parsed) {
    const status = config.strict ? 'no-remote' : 'no-remote';
    return { status, identity, remote: null, url, configPath, config, strict: config.strict };
  }

  const rule = matchRule(config.rules, parsed);
  if (!rule) {
    return { status: 'no-rule', identity, remote: parsed, url, configPath, config, strict: config.strict };
  }

  const expected = resolveIdentity(rule, config);
  if (!expected) {
